...
function output(laureate) {
    return {
        name: _.get(laureate, 'fullName.en').replace(/^\s+|\s+$/g, ''),
        country: _.get(laureate, 'birth.place.country.en').replace(/^\s+|\s+$/g, '')
    };
}

class NobelPrizeLaureatesParser {
    constructor(data) {
...
module.exports = NobelPrizeLaureatesParser;
